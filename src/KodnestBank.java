...
public class KodnestBank implements Bank {
	...
	private static KodnestBank ref = null;
...
//making singleton class and making constructor private
	...
	public void register() {
		...
		String query = "insert into kodnestbank values(?,?,?,?,?,?,?)";
...
//		int phone = scan.nextInt();
		...
		try {
			if (flag == true && AccNo.length() == 9 && password.length() >= 6 && phone.length() == 10 && amount >= 1000
				...
				System.out.println("Registration successfull.. welcome to kodnestBank.");
	...
	public void logIn() {
		...
		String query1 = "select amount from kodnestbank where accno='" + enteredAccNo + "' and password='"
				+ enteredPassword + "'";
		try {
...
//			e.printStackTrace();
	...
	public void Transfer() {
		...
		String q1 = "update kodnestbank set amount=amount-? where accno=? and password=?";
		String q2 = "update kodnestbank set amount=amount+? where accno=?";
		String CheckBalanceQuery = "select (amount-?) from kodnestbank where accno=? and password=?";
			...
			if (result < 0) {
				...
				try {
					...
					if (rowsAffected1 == 1 && rowsAffected2 == 1) {
						...
						System.out.println("Pleasee enter valid credentials");
						con.rollback();
					}
...
//					e1.printStackTrace();
	...
	public void ChangePassword() {
		...
		String query1 = "select password from kodnestbank where accno=? and password=?";
		try {
...
//			System.out.println("Inside two flag true");
			String setQuery = "update kodnestbank set password=? where accno=? and password=?";
...
//				e.printStackTrace();
	...
	public void deleteAccount() {
		...
		if (confirm == true) {
			...
			System.out.println("Enter your password:");
			String ePassword = scan.next();
			String query1 = "delete from kodnestbank where accno=? and password=?";
			try {
	...
	public void viewProfile() {
		...
		String query1 = "select * from kodnestbank where accno=? and password=?";
		try {
			PreparedStatement ps1 = con.prepareStatement(query1);
			ps1.setString(1, eAccNo);
			ps1.setString(2, ePassword);
			...
			if (flag == true) {
				...
				System.out.println("************KodnestBank***************\n");
